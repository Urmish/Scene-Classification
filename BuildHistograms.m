...
%	D = sp_dist2(X, C) takes two matrices of vectors and calculates the
...
%	Jth row of C.
    ...
    if ndata <= batchSize
        ...
        [min_dist, min_ind] = min(dist_mat, [], 2); %dist_mat, [], 2 returns the min value in each row, min_ind indiates the column at which row was found
        texton_ind.data = min_ind; %What would this indicate? sp_dist2 calculates a squared distance of each point 
        %So for each feature point in an image, it is calculating its
        %nearest neighbor in dictionary!!
    else
        for j = 1:batchSize:ndata
            ...
            %Its just that it is doing so in batches - So for each feature point in an image, it is calculating its
            %nearest neighbor in dictionary!!
        end
    end

    H = hist(texton_ind.data, 1:params.dictionarySize); %For each image, it is creating a histogram at base level
    H_all = [H_all; H]; %Question - Why is it concatenating the histogram?? I guess it will index accordingly

    %% save texton indices and histograms
    sp_make_dir(outFName);
    save(outFName, 'texton_ind'); %texton_ind.data - the dictionary cluster that the feature is closest to
    %texton_ind.x/y - x and y coordinate of the feature
    save(outFName2, 'H');
end
